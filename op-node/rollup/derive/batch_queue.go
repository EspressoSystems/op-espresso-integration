...
func NewBatchQueue(log log.Logger, cfg *rollup.Config, prev NextBatchProvider, l1 EspressoL1Provider, l2 SafeBlockFetcher) *BatchQueue {
	return &BatchQueue{
		log:    log,
		config: cfg,
		prev:   prev,
...
func (bq *BatchQueue) AddBatch(ctx context.Context, sysCfg *eth.SystemConfig, batch Batch, parent eth.L2BlockRef) {
	...
	validity := CheckBatch(ctx, sysCfg, bq.config, bq.log, bq.l1Blocks, parent, &data, bq.l1, bq.l2)
...
batchLoop:
	for i, batch := range bq.batches {
		validity := CheckBatch(ctx, sysCfg, bq.config, bq.log.New("batch_index", i), bq.l1Blocks, parent, batch, bq.l1, bq.l2)
		switch validity {
		case BatchFuture:
