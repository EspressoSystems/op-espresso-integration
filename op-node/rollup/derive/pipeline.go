...
func NewDerivationPipeline(log log.Logger, rollupCfg *rollup.Config, l1Fetcher L1Fetcher, l1Blobs L1BlobsFetcher, l2Source L2Source, engine LocalEngineControl, metrics Metrics, syncCfg *sync.Config) *DerivationPipeline {
	...
	if rollupCfg.HotShotContractAddress != nil {
		espressoProvider = NewEspressoProvider(log, *rollupCfg.HotShotContractAddress, l1Fetcher)
	}
	// Pull stages
	...
	batchQueue := NewBatchQueue(log, rollupCfg, chInReader, espressoProvider, l2Source)
