...
parser.add_argument(
    "--allocs",
    help="Only create the allocs and exit",
    type=bool,
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--test",
    help="Tests the deployment, must already be deployed",
    type=bool,
    action=argparse.BooleanOptionalAction,
)
...
parser.add_argument(
    "--l2-provider-url",
    help="URL for the L2 RPC node",
    type=str,
    default="http://localhost:19545",
)
parser.add_argument(
    "--deploy-l2",
    help="Deploy the L2 onto a running L1 and sequencer network",
    type=bool,
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--deploy-config",
    help="Deployment config, relative to packages/contracts-bedrock/deploy-config",
    default="devnetL1.json",
)
parser.add_argument(
    "--deploy-config-template",
    help="Deployment config template, relative to packages/contracts-bedrock/deploy-config",
    default="devnetL1-template.json",
)
parser.add_argument(
    "--deployment",
    help="Path to deployment output files, relative to packages/contracts-bedrock/deployments",
    default="devnetL1",
)
parser.add_argument(
    "--devnet-dir",
    help="Output path for devnet config, relative to --monorepo-dir",
    default=".devnet",
)
parser.add_argument(
    "--espresso",
    help="Run on Espresso Sequencer",
    type=bool,
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--compose-file",
    help="Compose file to use for demo images",
    type=str,
...
def main():
    ...
    devnet_config_path = pjoin(
        contracts_bedrock_dir, "deploy-config", args.deploy_config
    )
    devnet_config_template_path = pjoin(
        contracts_bedrock_dir, "deploy-config", args.deploy_config_template
    )
...
def deploy_contracts(paths, deploy_config: str, deploy_l2: bool):
    ...
    if deployer_balance == 0:
        log.info(f"No balance detected for create2 deployer, deploying create2...")
    ...
    deploy_env = {"DEPLOYMENT_CONTEXT": deploy_config.removesuffix(".json")}
    if deploy_l2:
...
def devnet_l1_genesis(paths, deploy_config: str):
    ...
    if geth_up:
        raise Exception(
            "Existing process is listening on localhost:8545, please kill it and try again. (e.g. `pkill geth`)"
        )
...
def devnet_deploy(paths, args):
    ...
    if args.deploy_l2:
        ...
        run_command(
            ...
            env={"PWD": paths.ops_bedrock_dir, "DEVNET_DIR": paths.devnet_dir},
        )
        wait_up(8545)
        wait_for_rpc_server("127.0.0.1:8545")
        ...
        if espresso:
            ...
            run_command(
                ["docker-compose", "-f", compose_file, "up", "-d"] + espresso_services,
                cwd=paths.ops_bedrock_dir,
                env={"PWD": paths.ops_bedrock_dir, "DEVNET_DIR": paths.devnet_dir},
            )
    ...
    if (
        not espresso
        and os.path.exists(paths.genesis_l2_path)
        ...
        run_command(
            [
                ...
                "--outfile.l2",
                pjoin(paths.devnet_dir, "genesis-l2.json"),
                "--outfile.rollup",
                pjoin(paths.devnet_dir, "rollup.json"),
            ],
            cwd=paths.op_node_dir,
        )
    ...
    run_command(
        ["docker-compose", "-f", compose_file, "up", "-d", f"{l2}-blockscout"],
        cwd=paths.ops_bedrock_dir,
    )
