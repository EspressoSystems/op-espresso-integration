...
// (p1, ... pn) against hotshot commitments h1,...hn.
//
// In other words, the function validates that txns = {...p1.txns, ..., ...pn.txns}. And that
// p1, ..., pn are all valid NMT proofs with respect to r1, ..., rn, the NMT roots of each commitment.
func ValidateBatchTransactions(transactions []hexutil.Bytes, nmtProofs []NmtProof, comms []Commitment) error {
	return nil
}
